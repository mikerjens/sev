(() => {
  ...
  function classify(hour){
    ...
    if(
      ...
      gust <= 14 &&
      visibility >= 5000 &&
      ![45,48,55,65,75,82,86].includes(code)
    ){
      return "good";
    }
  ...
  function longestWindow(hours,allowed){
    ...
    for(const hour of hours){
      ...
      if(
        current.length &&
        minutes(hour.time) !== minutes(current[current.length - 1].time) + 60
      ){
        current = [];
      }
  ...
  function formatWindow(hours){
    ...
    return `${start} to ${endHour}:${endMinute}`;
  }
  ...
  function assess(date,sunrise,sunset,hours){
    ...
    const daylight = hours
      .filter(hour =>
        ...
        minutes(hour.time) >= start &&
        minutes(hour.time) <= end
      )
      .map(hour => ({...hour,rating:classify(hour)}));

    const good = longestWindow(daylight,["good"]);
    if(good.length >= 3){
      return {rating:"good",window:formatWindow(good),length:good.length};
    }

    const usable = longestWindow(daylight,["good","caution"]);
    if(usable.length >= 3){
      return {rating:"caution",window:formatWindow(usable),length:usable.length};
    }
  ...
  function aggregate(forecasts,date){
    const regions = forecasts.map(forecast => {
      ...
      return {
        ...
        assessment:assess(
          date,
          forecast.data.daily.sunrise[index],
          forecast.data.daily.sunset[index],
          forecast.hours
        ),
    ...
    const best = [...regions].sort((a,b) =>
      rank[b.assessment.rating] - rank[a.assessment.rating] ||
      b.assessment.length - a.assessment.length
    )[0];
  ...
  function openWeather(date){
    ...
    requestAnimationFrame(() => {
      ...
      if(card){
        ...
        document.getElementById("panel-weather")?.scrollIntoView({behavior:"smooth",block:"start"});
      }
    });
  }
  ...
  function renderShortcut(days){
    ...
    container.innerHTML = days.map(day => {
      ...
      return `
        <button class="shortcut-day ${day.rating}${isToday ? " today" : ""}" type="button" data-weather-date="${esc(day.date)}" aria-label="${esc(dayFormat.format(dateObject))} ${esc(shortcutLabel(day.rating))}">
  ...
  function renderDetails(days,forecastCount){
    ...
    if(updatedElement){
      updatedElement.textContent = `Updated ${updated} · ${forecastCount}/6 regions`;
    }
  }
  ...
  async function fetchRegion(location){
    ...
    if(!data.daily || !data.hourly){
      throw new Error(`${location.name}: incomplete data`);
    }
  ...
  async function loadWeather(){
    ...
    try{
      ...
      if(forecasts.length < 3){
        throw new Error("Too few regional forecasts are available");
      }

      const dates = forecasts[0].data.daily.time;
      const days = dates.map(date => ({date,...aggregate(forecasts,date)}));
      ...
      if(shortcut){
        shortcut.innerHTML = `<div class="shortcut-error">Forecast temporarily unavailable</div>`;
      }
      ...
      const updatedElement = document.getElementById("weather-updated");
      if(updatedElement){
        updatedElement.textContent = "Forecast temporarily unavailable";
      }
    }
  }
