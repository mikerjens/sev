(() => {
  ...
  const MICHAEL_TASK_ID = 'task-elduvik-airbnb-permission';
  const MICHAEL_DATE_ID = 'task-elduvik-airbnb-permission-date';
  const ROOM_PENDING_ID = 'room-scenes-pending';
  const ROOM_SCENES = ['1A', '2A', '2B', '2C', '15A', '16A'];
  const ROOM_COMMENT = 'Scenerne planlægges ved et Airbnb-hus i Elduvík. Michael skal spørge ejeren, om filmoptagelse er mulig. Der fastsættes først en optagedato, når ejeren har godkendt optagelsen. Huset skal også vurderes i forhold til vinduerne i scene 9.';

  const people = {
    all: { label: 'Alle opgaver', role: 'Samlet produktionsplan', aliases: [] },
    michael: { label: 'Michael Koba', role: 'Producer', aliases: ['Michael Koba'] },
    thomas: { label: 'Thomas Koba', role: 'Instruktør og filmmaker', aliases: ['Thomas Koba'] },
    elisabeth: {
      label: 'Elisabeth Vitalis Tausen', role: 'SANSIR',
      aliases: ['Elisabeth Vitalis Tausen', 'Tór Verland Johannesen', 'Tór Verland Johansen', 'Bogi Henriksen']
    },
    tor: {
      label: 'Tór Verland Johannesen', role: 'SANSIR',
      aliases: ['Elisabeth Vitalis Tausen', 'Tór Verland Johannesen', 'Tór Verland Johansen', 'Bogi Henriksen']
    },
    bogi: {
      label: 'Bogi Henriksen', role: 'SANSIR',
      aliases: ['Elisabeth Vitalis Tausen', 'Tór Verland Johannesen', 'Tór Verland Johansen', 'Bogi Henriksen']
    }
  };
  ...
  function ownerText(card) {
    if (card.dataset.personId && people[card.dataset.personId]) return people[card.dataset.personId].label;
    const chips = Array.from(card.querySelectorAll('.task-chip.owner'));
    const text = chips.length ? chips.map(node => node.textContent.trim()).join(' · ') : card.textContent;
  ...
  function elisabethTaskMarkup() {
    return `
      ...
      <article class="task-card high" id="${ELISABETH_TASK_ID}" data-version="${VERSION}" data-person-id="elisabeth">
        ...
        <div class="task-meta">
          <span class="task-chip">Scene 4A</span><span class="task-chip">Casting</span>
          <span class="task-chip">Høj prioritet</span><span class="task-chip owner">Elisabeth Vitalis Tausen</span>
        </div>
  ...
  function michaelTaskMarkup() {
    return `
      <div class="plan-date" id="${MICHAEL_DATE_ID}" data-version="${VERSION}">Så snart som muligt · Elduvík</div>
      <article class="task-card high" id="${MICHAEL_TASK_ID}" data-version="${VERSION}" data-person-id="michael">
        <div class="task-top">
          <div>
            <div class="task-title">Spørg Airbnb-ejeren i Elduvík om filmtilladelse</div>
            <div class="task-time">Før optagedatoen for 1A, 2A, 2B, 2C, 15A og 16A fastsættes</div>
          </div>
          <span class="task-status">Ikke startet</span>
        </div>
        <div class="task-meta">
          <span class="task-chip">1A · 2A · 2B · 2C · 15A · 16A</span>
          <span class="task-chip">Location</span><span class="task-chip">Høj prioritet</span>
          <span class="task-chip owner">Michael Koba</span>
        </div>
        <div class="task-copy"><b>Opgave:</b> Kontakt ejeren af Airbnb-huset i Elduvík og spørg, om produktionen må filme indendørs og ved huset. Afklar adgang, mulige optagetider, antal personer, udstyr, parkering, strøm og eventuelle vilkår eller betaling.</div>
        <div class="task-done"><b>Færdig når:</b> Ejeren har givet en tydelig godkendelse, og de praktiske vilkår er dokumenteret, så optagedatoen kan fastsættes.</div>
      </article>`;
  }

  function ensureTask(list, taskId, dateId, markup) {
    const existing = document.getElementById(taskId);
    if (existing?.dataset.version === VERSION && document.getElementById(dateId)) return;
  ...
  function roomPendingMarkup() {
    return `
      <article id="${ROOM_PENDING_ID}" class="next-shoot-event next-shoot-pending" data-version="${VERSION}">
        <div class="next-shoot-date"><span>Dato afventer</span><b>EJERTILLADELSE AFVENTER</b></div>
        <div class="next-shoot-scenes">${ROOM_SCENES.map(scene => `<strong>${scene}</strong>`).join('')}</div>
        <h4>Airbnb i Elduvík · Drengens værelse</h4>
        <p>Michael spørger ejeren, om produktionen må filme der. Optagedato fastsættes først efter ejerens godkendelse.</p>
      </article>`;
  }
  ...
  function ensureRoomCalendarStatus() {
    ...
    calendar.querySelectorAll('.next-shoot-event').forEach(event => {
      const sceneCodes = Array.from(event.querySelectorAll('.next-shoot-scenes strong')).map(node => node.textContent.trim());
    ...
    const text = '1A · 2A · 2B · 2C · 15A · 16A · 9A–9C · 10A · 12A · 13A–13B · 14A';
    if (undated && undated.textContent !== text) undated.textContent = text;
  }
  ...
  function ensureRoomProducerStatus() {
    document.querySelectorAll('.producer-location-card').forEach(card => {
      ...
      const statusText = 'Afventer tilladelse fra ejer · ingen optagedato';
  ...
  function ensureRoomLegacyMilestone() {
    document.querySelectorAll('.milestone').forEach(milestone => {
      ...
      const titleText = 'Airbnb i Elduvík · afventer ejertilladelse';
      const bodyText = 'Scener 1A, 2A, 2B, 2C, 15A og 16A filmes samlet, når ejeren har godkendt optagelsen.';
      if (title && title.textContent !== titleText) title.textContent = titleText;
      if (text && text.textContent !== bodyText) text.textContent = bodyText;
      const date = milestone.previousElementSibling;
      if (date?.classList.contains('plan-date') && date.textContent.includes('9. august')) date.textContent = 'Dato afventer · ejertilladelse';
    });
  }
  ...
  function bindSelector() {
    ...
    if (!people[selectedId] || !Array.from(select.options).some(option => option.value === selectedId)) selectedId = 'all';
    select.value = selectedId;
    select.addEventListener('change', event => {
      selectedId = people[event.target.value] ? event.target.value : 'all';
  ...
  function ensurePersonalHeading() {
  ...
  function placeAfter(parent, node, previous) {
    ...
    } else if (previous.nextElementSibling !== node) previous.insertAdjacentElement('afterend', node);
    return node;
  }
  ...
  function ensureScheduleOrder() {
    ...
    if (!main || !list) return;
    const heading = ensurePersonalHeading();
  ...
  function updateSummary(visibleCount, totalCount) {
    ...
    const headingText = selectedId === 'all' ? 'Samlet opgaveplan' : `${person.label} · personligt skema`;
  ...
  function applyFilter() {
    ...
    } else if (empty) setVisible(empty, false);
  ...
  function sync() {
    ...
    ensureTask(list, ELISABETH_TASK_ID, ELISABETH_DATE_ID, elisabethTaskMarkup);
    ensureTask(list, MICHAEL_TASK_ID, MICHAEL_DATE_ID, michaelTaskMarkup);
