(() => {
  ...
  function contactMarkup(member) {
    const links = [];
    if (member.email) links.push(`<a href="mailto:${escapeHtml(member.email)}">✉ ${escapeHtml(member.email)}</a>`);
    if (member.phone) links.push(`<a href="tel:${member.phone.replace(/\s+/g, '')}">☎ ${escapeHtml(member.phone)}</a>`);
    if (member.website) links.push(`<a href="${escapeHtml(member.website)}" target="_blank" rel="noopener">↗ ${escapeHtml(member.website.replace(/^https?:\/\//, ''))}</a>`);
    return links.length ? `<div class="team-contact-list">${links.join('')}</div>` : '';
  }

  function installStyles() {
    if (document.getElementById('sev-team-v3-styles')) return;
    const style = document.createElement('style');
    style.id = 'sev-team-v3-styles';
    style.textContent = `
  ...
  function installTabMemory() {
    ...
    window.setTimeout(() => {
      ...
      if (button && panel && typeof window.openPortalTab === 'function') window.openPortalTab(saved);
    }, 400);
  }
