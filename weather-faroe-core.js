(() => {
  ...
  if (weatherShell && schedulePanel) {
    schedulePanel.prepend(weatherShell);
  }

  const style = document.createElement("style");
  style.textContent = `
    #panel-schedule .weather-shell{
      max-width:none;
      margin:0 0 34px;
      padding:0 0 30px;
    ...
    @media(max-width:600px){
      #panel-schedule .weather-shell{margin-bottom:28px;padding-bottom:24px}
      .weather-grid{display:flex!important}
      .weather-card{min-width:178px}
    }
  `;
  ...
  if (subtitle) {
    subtitle.textContent = "Faroe Islands. Automatically assessed across six production regions.";
  }
  ...
  function render(forecasts) {
    ...
    if (updatedElement) {
      updatedElement.textContent = `Updated ${updated} · ${forecasts.length}/6 regions`;
    }
  }

  async function fetchRegion(location) {
    const response = await fetch(`${apiUrl(location)}&_=${Date.now()}`, {
      cache: "no-store"
    });
